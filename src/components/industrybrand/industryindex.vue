<template>
  <div class="brindex">
    <div class="brheader active">
      <ul class="brnav">
        <router-link tag="li"  :to="{path:'/module/brindex'}">
          品牌维权
         </router-link>
        <router-link tag="li" class="active" :to="{path:'/module/industryindex'}">
          行业情报
        </router-link>
      </ul>
      <div class="search">
    ...
    methods:{
      path:function () {
        if(this.val==''){
          ...
          this.$router.push({path: '/module/indseach',query: {vals: this.val}});
        }
      }
    },
    created(){
